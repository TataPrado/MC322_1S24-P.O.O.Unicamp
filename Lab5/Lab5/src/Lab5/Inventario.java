...
public class Inventario{
    ...
    public boolean adicionarItem(Item item) {
        ...
        if (itens.size() <= limiteDeltens) {
            if (item.getItemGrande() == true) {
                System.out.println("Você já possui um Item Grande. Não é possível adicionar mais um.");
                return false;
            } else {
                itens.add(item);
            ...
            System.out.println("Limite de itens atingido. Não foi possível adicionar novo item ao inventário");
            return false;
        }
    }
    ...
    public void removerItem(String nomeItem){
        Iterator<Lab5.Item> iterator = itens.iterator();
